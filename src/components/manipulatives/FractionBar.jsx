...
function Face({ taken, compact, width, expression = 'idle', blinkProgress = 1 }) {
  const eyeOffsetX = Math.max(0.08, width * 0.16)
  const eyeY = compact ? 0.04 : 0.06
  const eyeWidth = compact ? 0.035 : 0.045
  const eyeHeight = Math.max(0.01, (compact ? 0.03 : 0.04) * blinkProgress)
  const pupilSize = compact ? 0.012 : 0.014
  const mouthY = compact ? -0.06 : -0.08

  const isSurprise = expression === 'surprise'
  const isHappy = expression === 'happy' || taken
  const isConfused = expression === 'confused'
  const browTilt = isSurprise ? -0.25 : isConfused ? 0.3 : isHappy ? 0.12 : 0

  const armY = compact ? -0.02 : 0.01
  const armX = width / 2 + 0.04
  const footY = -BAR_H / 2 - 0.06
  const limbColor = taken ? '#fff4df' : '#ffffff'

  return (
    <group position={[0, 0, FACE_Z]}>
      {!compact && (
        <>
          <mesh position={[-armX, armY, -0.02]} rotation={[0, 0, 0.55]}>
            <capsuleGeometry args={[0.018, 0.08, 4, 8]} />
            <meshStandardMaterial color={limbColor} roughness={0.75} metalness={0.03} />
          </mesh>
          <mesh position={[armX, armY, -0.02]} rotation={[0, 0, -0.55]}>
            <capsuleGeometry args={[0.018, 0.08, 4, 8]} />
            <meshStandardMaterial color={limbColor} roughness={0.75} metalness={0.03} />
      ...
      <mesh position={[-eyeOffsetX, eyeY, 0.01]}>
        <sphereGeometry args={[eyeWidth, 14, 14]} />
      ...
      <mesh position={[-eyeOffsetX, eyeY, 0.022]} scale={[1, eyeHeight / eyeWidth, 1]}>
        <sphereGeometry args={[pupilSize, 10, 10]} />
        <meshStandardMaterial color="#13213d" roughness={0.35} metalness={0} />
      </mesh>
      <mesh position={[eyeOffsetX, eyeY, 0.022]} scale={[1, eyeHeight / eyeWidth, 1]}>
        <sphereGeometry args={[pupilSize, 10, 10]} />
        <meshStandardMaterial color="#13213d" roughness={0.35} metalness={0} />
      ...
      <mesh position={[-eyeOffsetX, eyeY + 0.07, 0.02]} rotation={[0, 0, browTilt]}>
        <boxGeometry args={[0.09, 0.012, 0.008]} />
        <meshStandardMaterial color="#22314f" roughness={0.5} metalness={0} />
      </mesh>
      <mesh position={[eyeOffsetX, eyeY + 0.07, 0.02]} rotation={[0, 0, -browTilt]}>
        <boxGeometry args={[0.09, 0.012, 0.008]} />
        <meshStandardMaterial color="#22314f" roughness={0.5} metalness={0} />
      ...
      {isSurprise ? (
        <mesh position={[0, mouthY, 0.021]}>
          <torusGeometry args={[0.03, 0.011, 10, 20]} />
          <meshStandardMaterial color="#a61c3c" roughness={0.45} metalness={0} />
        </mesh>
      ) : (
        <mesh position={[0, mouthY, 0.021]} rotation={[0, 0, isHappy ? 0 : Math.PI]}>
          <torusGeometry args={[0.06, 0.008, 8, 24, Math.PI]} />
          <meshStandardMaterial color={isHappy ? '#ff4d6d' : '#2f3b52'} roughness={0.45} metalness={0} />
        </mesh>
      )}

      {!compact && (
        <>
          <mesh position={[-width * 0.2, footY, -0.02]}>
            <capsuleGeometry args={[0.02, 0.06, 4, 8]} />
            <meshStandardMaterial color={limbColor} roughness={0.78} metalness={0.02} />
          </mesh>
          <mesh position={[width * 0.2, footY, -0.02]}>
            <capsuleGeometry args={[0.02, 0.06, 4, 8]} />
            <meshStandardMaterial color={limbColor} roughness={0.78} metalness={0.02} />
